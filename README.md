...
Answer:  Context API is useful when props are requested by many components across the application or deep in the component tree where prop drilling would not be effective?
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Answer:  Actions are packets of information describing events that occurred in the UI.  Reducers are pure functions that take in current state and action as arguments. Reducers uses a switch case to check the action type and action per/load. Each case in the switch statement returns the newly updated state thus triggering the UI to re-render with new data.  Store is a giant JSON object. Store saves and recovers current state of an application. Store is also the only place where data is stored.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Answer:  Application state is global or for the entire application while Component state is for state within that particular component. Application state is useful when various components across the application share the same data.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Answer:  Redux-Think is a middleware that provides the ability to handle asynchronous operations inside Action Creators. When an action creator is called, redux-think intercepts and acts on returned data. If action, it's sent to reducer, but if it's a function(thunk), the thunk is invoked and passes the dispatch function as an argument to it.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
Answer:  My favorite would be Context API because it is simple to use and requires no external dependencies.
